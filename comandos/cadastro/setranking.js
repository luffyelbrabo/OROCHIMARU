...
⚠️ *VALE LEMBRAR QUE O USO DOS DOIS PONTOS ( : ) É OBRIGATÓRIO!*`;
    ...
    if (isNaN(vila)) {
      return sock.sendMessage(m.chat, { text: '❌ Número da vila inválido. Exemplo: *setranking Seu Nome:320*' }, { quoted: m });
    }
    ...
    db.run(
      ...
      (err) => {
        if (err) {
          return sock.sendMessage(m.chat, { text: '❌ Erro ao cadastrar sua vila no ranking.' }, { quoted: m });
        }
